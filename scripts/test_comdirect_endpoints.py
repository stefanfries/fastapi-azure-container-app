            ...
            if response.status_code == 200:
                ...
                if currency_meta:
                    ...
                    for meta in all_metas[:10]:  # Show first 10
                        print(
                            f"    - itemprop='{meta.get('itemprop')}' content='{meta.get('content')}'"
                        )
...
async def test_with_different_instruments():
    ...
    print(f"\n{'=' * 80}")
    print("TEST 3: Testing with different instruments")
    print(f"{'=' * 80}")
    ...
    for instrument in test_instruments:
        ...
        try:
            async with httpx.AsyncClient(follow_redirects=True) as client:
                ...
                print(
                    f"  {status} Status: {response.status_code}, Length: {len(response.content)} bytes"
                )
